...
option_list <- list(                   
	...
	make_option("--mu",  type="double",   default=0,help=("mu0 (%default)"),metavar="shifts"),
	...
	for (i in 1:ngen) {
		...
		if (rr[1]<update_sig_freq) {
			...
			if (RUNIF[1]<bdmcmc_freq){
				...
				bdmcmc_list   = run_BDMCMC(tree, trait_states, sig2.prime,ind_sig2, mu0.prime, ind_mu0,D,IND_edge,new_ind,desc_index_list,death_sig2=sample(2,1))
...
#################### RUN SIMULATION
resfile = sprintf("sim_%s_s2_%s_mu0_%s_x_%s.pdf", rep, sig2, mu0, xfold)
pdf(file=resfile,width=0.9*15, height=1.05*15)
par(mfrow=c(4,3))
#S = sim_data(ntips=ntips,s2=sig2,xfold=xfold,n_shifts=n_shifts)
#S = sim_data_bmt(ntips=ntips,s2=0.1,m0=0.75,root_value=0)
S = sim_data_bmt_shifts(s2=sig2,mu0=mu0,xfold_sig2=xfold,n_shifts_sig2=n_shifts,n_shifts_mu0=n_shifts_mu0)
#S = sim_data(s2=sig2,xfold=xfold,n_shifts=n_shifts)
tree= S[[1]]
ntips=tree$Nnode+1
full_data= S[[2]]
data <- S[[2]][names(S[[2]]) %in% tree$tip.label]
...
print("Starting MCMC...")
t1 = get_time()
logfile3 = sprintf("sim_%s_n_%s_s2_%s_x_%s_mu_%s_%s.log", rep, ntips, sig2, xfold, mu0, n_shifts_mu0)
